...
UNBOUNDED PRECEDING - 현재 행을 기준으로 선행(이전)하는 모든 행들 -- 기준으로 이전에 있는 행
CURRENT ROW - 현재 행
UNBOUNDED FOLLOWING - 현재 행을 기준으로 이후 모든 행들	-- 기준으로 이후에 있는 행

RANGE UNBOUNDED PRECEDING : 현재 행을 기준으로 해서 파티션 내의 첫번째 행까지 범위를 지정한다.
ROWS BETWEEN 1 PRECEDING AND 1 FOLLOWING : 현재 행을 기준으로 파티션 내 앞뒤 한건씩을 범위로 지정
...
SELECT empno, ename, deptno, sal,
		sum(sal) OVER(ORDER BY sal ROWS UNBOUNDED PRECEDING) c_sum1,
...
SQL 은 집합적이지만 실생활에서 발생하는 요구사항을 처리하기 워해서는 절차적 처리가 필요할 때가 있음

PL/SQL 블록의 기본구조
DECLARE - 상수, 변수를 선언, 선언이 필요없을 경우 생략가능
		- java와 다르게 지역변수를 선언할 수 없음
BEGIN - 로직을 서술하는 부분
EXCEPTION - 예외 발생시 에외 처리 기술(생략가능)

화면 출력기능을 활성화 하는 설정(oracle 접속 후 최초 1회만 실행하면 접속종료시까지 유효)
SET SERVEROUTPUT ON;	-- sqlplus, sqldevelper 에서 사용

----------------------------------------------------------------------------------------

간단한 PL/SQL 익명(이름이 없는) 블럭
dept 테이블에서 10번 부서의 deptno, dname 두개의 컬럼의 값을 조회해서 변수에 담아 화면 출력

java 변수 선연 - 변수타입 변수명 = 10;
...
-- PL/SQL 블록 내에서는 한문장이 종료될 때마다 세미콜론(;)을 사용하여 한문장이 끝났다는 것을 명시

-------------------------------------------------------------------------------------------------

변수 참조 타입
	deptno NUMBER(2) ==> deptno dept.deptno%TYPE;	-- dept의deptno의 타입과 동일하게 참조하겠다는 뜻
	테이블에 컬럼 타입 변경이 생겨도 PL/SQL 코드는 수정할 필요가 없어진다.
...
JAVA 메서드 - public String 함수명(인자타입 인자명1, 인자타입2, 인자명2, ...)
PL/SQL 인자 선언 - 프로시져명(인자명1 IN 인자타입1, 인자명2 IN 인자타입2,......)
		인자명은 - p_ 접두어를 주로 사용
		변수명은 - v_ 접두어를 주로 사용
		
1. 인자를 받게끔 수정 ( X ==> 부서본호를 인자로 받는다.)
