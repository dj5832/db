-- sal 를 1000으로 나눴을 때의 나머지 ==> MOD 함수, 별도의 연산자는 없다.
...
TO_DATE('2023/12/31', 'YYYY/MM/DD') lastday, TO_DATE('2023/12/31', 'YYYY/MM/DD') -5 
  FROM dual;
...
SELECT SYSDATE now, TO_CHAR(SYSDATE, 'YYYY-MM-DD') now_format
	  ...
	  ,TO_CHAR(SYSDATE, 'WW') "1일에서 7일까지"
	  ,TO_CHAR(SYSDATE, 'w')-- "매월1일을기준으로한주차"
  FROM dual;
...
-- MONTHS_BETWEEN
SELECT ename, TO_CHAR(hiredate, 'YYYY') hiredate,
	   MONTHS_BETWEEN(SYSDATE, hiredate) 
  FROM emp;
 
-- ADD_MONTHS
SELECT ADD_MONTHS(SYSDATE, 5) aft5
 	  ,ADD_MONTHS(SYSDATE, -5) bdf5
 FROM dual;
...
SELECT TO_CHAR(0.401, 'FM9990.999')
  FROM dual;
-- FM9990,99 의 의미
...
-- NULL과 관련된 함수 : NULL 값을 다른값으로 치환 하거나, 혹은 강제로 NULL을 만드는 것
-- 1. NVL(expr1, expr2)
--      if(expr1 == null)
--         expr2를 반환
--      else
...
--          expr1을 반환
--      else
--           COALESCE(expr2, .....)    ==> 재귀함수개념 (자기자신을 계속호출)
