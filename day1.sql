-- 별칭은 SQL내부에서 무조건 대문자로 표현되는데 소문자로 표현하고 싶으면 더블퀘테이션으로 묶는다.
...
--	언어마다 연산자 표기, literal 표기법이 다르기 때문에 해당 언어에서 지정하는 방식을 잘 따라야한다.
--	문자열 연산
--	java에서 문자열 결합연산자 : +
--	SQL에서 문자열 결합 연산자 : ||
...
-- member 테이블에서 회원의 이름에 글자[이]가 들어가는 모든 사람의 mem_id, mem_name 을 조회하는 쿼리를 작성해주세여
SELECT mem_id, mem_name
FROM MEMBER
