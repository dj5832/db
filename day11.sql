...
특이점 : 1. WHERE 절에서 사용
	    ...
	    3. 비상호연관 서브쿼리, 상호연관 서브쿼리 둘다 사용 가능하지만 주로 상호연관 서브쿼리[확인자] 와 사용된다.
...
가로 확장(컬럼을 확장) : JOIN 
세로 확장(행을 확장) : 집합연산
					집합연산을 하기 위해서는 연산에 참여하는 두개의 SQL(집합)이 *동일한 컬럼의 개수와 타입*을 가져야한다.
...
합집합 : UNION / UNION ALL
		...
		{1, 2, 3} U {1, 4, 5} ==> UNION ALL{1, 1, 2, 3, 4, 5} ==> UNION{1, 2, 3, 4, 5}
...
교환법칙 : 항의 위치를 수정해도 결과가 동일한 연산
		a + b = b + a
차집합의 경우 교환법칙이 성립되지 않음. 왜? ==> 뒤에있는 것을 빼는 것이기 때문
...
UNION ALL : 중복을 허용한다. 위의 집합과 아래집합을 단순히 합친다.
...
MINUS : 차집합 한 쪽 집합에서 다른 쪽 집합을 뺀 것;
...
SELECT mgr, ename -- 컬럼명의 결과가 mgr이 아닌 empno가 나온 것
  FROM emp
 WHERE empno IN (7566, 7698);
...
SELECT empno, ename
  FROM emp 
 WHERE empno IN(7566, 7698)
ORDER BY empno; -- 마지막에만 쓰면 된다. 아니면 오류남
