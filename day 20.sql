...
dept 테이블의 10번 부서 정보(3가지 컬럼)을 %ROWTYPE 으로 받아 화면에 출력 익명 블럭으로 작성하세요.
...
CURSOR 의 종류
묵시적 커서 : 커서 이름을 별도로 지정하지 않을 경우 ==> ORACLE이 알아서 처리 해줌
...
CURSOR 의 경우 반복문과 사용되는 일이 많기 때문에 PL/SQL 에서는 FOR LOOP 와 함께 사용하는 문법을 지원한다.
...
BEGIN
	FOR emp_row IN emp_cur(10) LOOP
		DBMS_OUTPUT.PUT_LINE(emp_row.deptno || ' , ' || emp_row.ename || ' , ' || emp_row.empno);
	END LOOP;
END;
/
