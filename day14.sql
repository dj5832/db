...
select *
 ...
 ALTER TABLE emp DROP CONSTRAINT SYS_C007101;
...
empno 컬럼의 인데스를 UNIQUE 인덱스가 아닌 일반 인덱스(중복이 가능한)로 생성한 경우
...
DBA_ ==> 데이터베이스에 설ㅊ된 모든 객체(DBA 권한이 있는 사용자만 가능 - system)
...
select *
  FROM dba_tables; -- system  계정에서만 조회 가능
...
실 사용 예 - 1. 실제 사용할 테이블과 별개로 보조 테이블에도 동일한 데이터 쌓기
		   ...
		   하나의 테이블안에 데이터값에 따라 저장하는 물리공간이 나눠어져 있음
...
2. emp 테이블의 empno 컬럼이랑 ename 컬럼만 갖고 emp_test, emp_test2를 생성
   단, 데이터를 복사하지 않음.
...
2. 테이블의 데이터를 이용하여 다른 테이블의 데이터를 업데이트 OR INSERT 하고 싶을 때
	...
	ALLEN 의 JOB과 DEPTNO를 SMITH 사원과 동일하게 업데이트 하시오.
	...
	emp 테이블에 동일한 empno가 있으면 ename을 업데이트하고 싶고
...
USING (테이블명 | VIEW | inine-VIEW ) alias
...
USING emp b
 ...
 WHEN NOT MATCHED THEN 
 INSERT (a.empno, a.ename) VALUES (b.empno, b.ename); -- INSERT (a.empno, a.ename) 은 없어도됨
...
--실습2
사용자가 입력받은 값을 이용한 MERGE 
사용자 입력 - deptno 99, dname 'dw', loc 'deajeon'
