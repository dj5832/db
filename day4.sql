...
--		WHERE ROWNUM BTWEEN 1 AND N;		ROWNUM이 1보다 크거나 같고 N보다 작거나 같은 경우
--  ***** 안되는 경우 *******
-- 		WHERE ROWNUM = 2;
-- 		WHERE ROWNUM >= 2;
-- ROWNUM 사용 용도 : 페이징 처리
...
-- 			  ==> 한 페이지당 건수를 정해놀고 해당 건수만큼만 조회해서 화면에 보여준다.
...
-- 		하나의 행이 입력되어서 결과로 하나의 행이 나온다.
-- 1. Multi row function 
-- 		여러개의 행이 입력되어서 결과로 하나의 행이 나온다.
...
SELECT CONCAT('Hello', CONCAT(',','World'))  concat
	 ...
	 , INSTR('Hello, World', 'o', 2) instr3
	 , INSTR('Hello, World', 'o', 1, 2) instr4  
	 , LPAD('Hello, World', 15, '@') lpad
	 , RPAD('Hello, World', 17, '!') rlad
...
-- emp 테이블에서 사원이름이 smith 인 사람을 조회하세요.
