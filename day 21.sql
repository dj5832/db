...
CREATE OR REPLACE TRIGGER make_history 
	BEFORE UPDATE ON users				-- users 가 업데이트 하기 전
	FOR EACH ROW 						-- 행 한줄한줄 전체 
...
CREATE OR REPLACE TRIGGER make_history 
	BEFORE UPDATE OF 컬럼명 ON users				-- users 가 업데이트 하기 전
	BEFORE INSERT OR UPDATE OR DELETE ON users
	FOR EACH ROW 						-- 행 한줄한줄 전체 감시
	WHEN (:OLD.usersid = 'brown')
	BEGIN
		/*
		 *  :OLD.컬럼명 ==> 기존값
		 *  :NEW.컬럼명 ==> 갱신값
