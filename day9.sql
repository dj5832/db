...
KING의 경우 PRESIDENT 이기 떄문에 mgr 컬럼의 값이 NULL ==> 조인에 실패
...
select e.empno, e.ename, m.empno, m.ename 
  FROM emp e, emp m
 WHERE e.mgr = m.empno(+);	-- 기준이 emp e 이므로 조회했을 때 안나오는 즉, NULL 로 채워지는 emp m 쪽에 (+)를 붙여야 한다.
 
OUTER JOIN 시 조인 조건(ON절에 기술)과 일반 조건(WHERE 절에 기술) 적용시 주의 사항
...
SELECT e.empno, e.ename, e.deptno, m.empno, m.ename
 ...
 ==> deptno = 10이 아닌 사람들의 데이터는 나오지 않는다.
 
ORACLE-SQL;
--위의 결과가 원하는 결과가 안나올 수 있다는 경우가 아래 퀴리
SELECT e.empno, e.ename, e.deptno, m.empno, m.ename, m.deptno 
  FROM emp e, emp m
 WHERE e.mgr = m.empno(+)
 AND m.deptno(+) = 10;
-- 추가 조건에도 (+)를 붙혀줘야 정확한 결과가 나온다
...
FULL OUTER : LEFT OUTER + RIGHT OUTER - 중복은 제거됨;
...
select e.empno, e.ename, m.empno, m.ename
  FROM emp e, emp m
 WHERE e.mgr(+) = m.empno(+);	-- 오라클 SQL은 FULL OUTER 문법을 제공하지 않음 쓰고 싶으면 ANSI-SQL써야함.
